...
DROP TABLE IF EXISTS muayene_kayitlari;
DROP TABLE IF EXISTS randevular;
DROP TABLE IF EXISTS personeller;
DROP TABLE IF EXISTS araclar;
DROP TABLE IF EXISTS muayene_turleri;
DROP TABLE IF EXISTS istasyonlar;
DROP TABLE IF EXISTS arac_sahipleri;

CREATE TABLE arac_sahipleri (
...
CREATE TABLE muayene_kayitlari (
...
BEGIN
    UPDATE arac_sahipleri SET ad=p_ad, soyad=p_soyad, telefon=p_telefon, eposta=p_eposta, adres=p_adres
...
BEGIN
    UPDATE araclar SET plaka=p_plaka, marka=p_marka, model=p_model, yil=p_yil, yakit_turu=p_yakit_turu, arac_tipi=p_arac_tipi
...
BEGIN
    SELECT a.arac_id, a.plaka, a.marka, a.model, a.yil, a.yakit_turu, a.arac_tipi,
           CONCAT(s.ad,' ',s.soyad) AS arac_sahibi
...
BEGIN
    UPDATE istasyonlar SET istasyon_adi=p_istasyon_adi, il=p_il, ilce=p_ilce, adres=p_adres, telefon=p_telefon
    WHERE istasyon_id=p_istasyon_id;
END $$
...
BEGIN
    UPDATE personeller SET istasyon_id=p_istasyon_id, ad=p_ad, soyad=p_soyad, gorev=p_gorev, telefon=p_telefon
...
BEGIN
    UPDATE muayene_turleri SET tur_adi=p_tur_adi, temel_ucret=p_temel_ucret, aciklama=p_aciklama
    WHERE muayene_tur_id=p_muayene_tur_id;
END $$
...
BEGIN
    UPDATE randevular SET arac_id=p_arac_id, istasyon_id=p_istasyon_id, muayene_tur_id=p_muayene_tur_id,
    randevu_tarih=p_randevu_tarih, randevu_saat=p_randevu_saat, durum=p_durum, aciklama=p_aciklama
...
BEGIN
    SELECT r.randevu_id, a.plaka, CONCAT(s.ad,' ',s.soyad) AS arac_sahibi,
           i.istasyon_adi, mt.tur_adi, r.randevu_tarih, r.randevu_saat, r.durum, r.aciklama
    FROM randevular r
...
BEGIN
    UPDATE muayene_kayitlari SET personel_id=p_personel_id, sonuc=p_sonuc, kusur_aciklama=p_kusur_aciklama
    WHERE muayene_id=p_muayene_id;
END $$
...
BEGIN
    SELECT mk.muayene_id, r.randevu_id, a.plaka, CONCAT(p.ad,' ',p.soyad) AS personel_ad_soyad,
           mk.sonuc, mk.kusur_aciklama, mk.muayene_tarihi
...
BEGIN
    UPDATE odemeler SET tutar=p_tutar, odeme_turu=p_odeme_turu, durum=p_durum
...
BEGIN
    SELECT o.odeme_id, r.randevu_id, a.plaka, CONCAT(s.ad,' ',s.soyad) AS arac_sahibi,
           o.odeme_tarihi, o.tutar, o.odeme_turu, o.durum
    FROM odemeler o
...
BEGIN
    ...
    SELECT COUNT(*) INTO randevu_sayisi
    FROM randevular
    ...
    IF randevu_sayisi > 0 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT='Bu tarih ve saat için randevu doludur.';
    END IF;
END $$
...
INSERT INTO muayene_turleri(tur_adi, temel_ucret, aciklama)
VALUES ('Periyodik Muayene',1800.00,'Standart araç muayenesi')
ON DUPLICATE KEY UPDATE temel_ucret=VALUES(temel_ucret);
